...
class CharacterRemoteDataSourceImpl(private val apiService: MarvelApiService) : CharacterRemoteDataSource {

    override suspend fun getCharacters(): Either<Failure, BaseResponse<CharacterResponse>> {
        val response = apiService.getCharacters()

        return if (response.isSuccessful) Either.Right(response.body()!!)
        else Either.Left(MarvelApi.parseError(response).getFailure())
    }
